...
export class MemberList implements OnInit {
  ...
  //signal deve ser inicializado no onInit
  ...
  constructor() {
    ...
    if (filters) {
      this.memberParams = JSON.parse(filters) as MemberParams
      this.updatedParams = JSON.parse(filters) as MemberParams
    }
  }
  ...
  get displayMessage(): string {
    ...
    if (this.updatedParams.minAge !== defaultParams.minAge || 
        this.updatedParams.maxAge !== defaultParams.maxAge) {
          filters.push(`ages between: ${this.updatedParams.minAge}-${this.updatedParams.maxAge}`)  
    }
